...
def _cfg(**tel_kw):
    ...
    return SimpleNamespace(
        ...
        simulator=SimpleNamespace(provider="google", api_key="x" * 30),
        reports_dir=None,
        scenarios_dir=None,
        dot_dir=None,
    )
...
async def test_preflight_warns_trunk_without_sim_inbound(tmp_path, monkeypatch):
    ...
    def fake_load(root, profile=None, environment=None):
        cfg = SimpleNamespace(
            ...
            observe=SimpleNamespace(timezone="UTC"),
            simulator=SimpleNamespace(provider="google", api_key="A" * 30),
