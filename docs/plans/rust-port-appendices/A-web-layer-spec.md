...
- **Acceptance gates:**
  - **(a) offline/CI**: all §6 tests green, including `parity_cues_golden` — byte-diff of `cues.json` against Python `web/cues.py` output for the R5 real run fixture (`114-people-pleaser-refuse-card-20260809-201652-8b32`, captured into `tests/golden/` before the phase, per cross-validation strategy 1) and for one in-progress report dir (live-fallback shape).
