...
pub const SUPPORTED_EFFECTS: [&str; 5] = ["breaking_up", "echo", "packet_loss", "phone_quality", "static"];
...
pub fn resolve_audio_effects(spec: Option<&serde_json::Value>) -> Result<Vec<Box<dyn Fn(&[u8]) -> Vec<u8> + Send>>, String> {
    let mut chain: Vec<Box<dyn Fn(&[u8]) -> Vec<u8> + Send>> = Vec::new();
    ...
    let entries: Vec<(String, serde_json::Value)> = match spec {
        ...
        serde_json::Value::Array(a) => a
            .iter()
            .filter_map(|v| v.as_str().map(|s| (s.to_string(), serde_json::Value::Bool(true))))
            .collect(),
        _ => return Ok(chain),
    };
    for (name, kw) in entries {
        ...
        match name.as_str() {
            "packet_loss" => {
                let probability = kwargs.get("probability").and_then(|v| v.as_f64()).unwrap_or(0.05);
                let chunk_ms = kwargs.get("chunk_ms").and_then(|v| v.as_i64()).unwrap_or(20) as usize;
                let chunk = (24_000usize * chunk_ms).max(1) / 1000;
                let mut rng = seed_rng(&["packet_loss", &probability.to_string(), &chunk.to_string()]);
                chain.push(Box::new(move |pcm: &[u8]| {
                    if probability <= 0.0 || pcm.is_empty() {
                        return pcm.to_vec();
                    }
                    let mut s = pcm_to_samples(pcm);
                    dropout(&mut s, chunk, probability, &mut rng);
                    samples_to_pcm(&s)
                }));
            }
            "breaking_up" => {
                let probability = kwargs.get("probability").and_then(|v| v.as_f64()).unwrap_or(0.2);
                let chunk = 24_000 * 100 / 1000;
                let mut rng = seed_rng(&["breaking_up", &probability.to_string(), &chunk.to_string()]);
                chain.push(Box::new(move |pcm: &[u8]| {
                    if pcm.is_empty() {
                        return pcm.to_vec();
                    }
            ...
            "echo" => {
                let delay_ms = kwargs.get("delay_ms").and_then(|v| v.as_i64()).unwrap_or(200) as usize;
                let decay = kwargs.get("decay").and_then(|v| v.as_f64()).unwrap_or(0.5);
                let delay_samples = 24_000usize * delay_ms / 1000;
                if delay_samples <= 0 {
                    return Err(format!("echo delay_ms must be positive (got {delay_ms})"));
                }
                chain.push(Box::new(move |pcm: &[u8]| {
                    if pcm.is_empty() {
                        return pcm.to_vec();
            ...
            "phone_quality" => {
                let window = (24_000usize / 6000).max(1);
                chain.push(Box::new(move |pcm: &[u8]| {
                    if pcm.is_empty() {
                        return pcm.to_vec();
                    }
            ...
            "static" => {
                let intensity = kwargs.get("intensity").and_then(|v| v.as_f64()).unwrap_or(0.05);
                if !(0.0..=1.0).contains(&intensity) {
                    return Err(format!("static intensity must be in [0.0, 1.0] (got {intensity})"));
                }
                let mut rng = seed_rng(&["static", &intensity.to_string()]);
                chain.push(Box::new(move |pcm: &[u8]| {
                    if pcm.is_empty() {
                        return pcm.to_vec();
                    }
                    let mut out = pcm_to_samples(pcm);
                    let amp = (32767.0 * intensity).round() as f64;
            ...
            other => {
                return Err(format!("unknown audio effect: {other} (supported: {})", SUPPORTED_EFFECTS.join(", ")));
            }
        }
    }
    Ok(chain)
...
pub fn apply_effects(chain: &[Box<dyn Fn(&[u8]) -> Vec<u8> + Send>], pcm: &[u8]) -> Vec<u8> {
